...
def dispatch_orders_to_drivers(id_to_unallocated_order: dict, id_to_driver: dict, id_to_location: dict, id_to_restaurant: dict):
    ...
    for order_id, order in id_to_unallocated_order.items():
        ...
        # initialize two dicts: {driver_id: distance}, {driver_id: driver capacity}
        ...
        # if order.pickup_location_id not in id_to_restaurant:
        #     logging.warning(f"order id is {order.id}, pickup location is {order.pickup_location_id}")
        
        
        customer_location_id = order.delivery_location_id
        # restaurant_location = id_to_location.get(restaurant_location_id)
        restaurant_location = id_to_restaurant.get(restaurant_location_id)
        
        # logging.warning(f"restaurant location id is {restaurant_location.id}")
        ...
        for driver_id, driver in id_to_driver.items():
            if driver_id in available_driver_ids:
                ...
                # driver in restaurant dispatch area
                if order_driver_distance <= restaurant_location.dispatch_radius:
        ...
        min_distance = min(order_driver_distance_dict.values()) # sometimes order_driver_distance_dict is empty
    ...
    for driver_id, driver in id_to_driver.items():
        ...
        # output the original planned route after greedy insertation
