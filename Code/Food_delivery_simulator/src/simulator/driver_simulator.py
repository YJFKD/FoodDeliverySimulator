...
class DriverSimulator(object):
    ...
    def loading_and_unloading(node, carrying_orders, completed_orders_ids, ongoing_orders_ids):
        ...
        骑手取餐和送餐，更新订单信息
        Inputs:
        - carrying_orders: orders now is carrying by driver
        ...
        # 对于在completed orders里面的订单，将其从ongoing orders里面删除
        for order_id in completed_orders_ids:
            if order_id in ongoing_orders_ids:
                ongoing_orders_ids.remove(order_id)
