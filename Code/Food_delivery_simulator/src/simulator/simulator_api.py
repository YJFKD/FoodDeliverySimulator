...
def __initialize(customer_info_file_name: str, restaurant_info_file_name:str, route_map, instance_folder: str):
    '''
    初始化模拟器
    ...
    '''
    # 获取文件绝对路径
    customer_location_info_file_path = os.path.join(Configs.medium_instance_folder_path, customer_info_file_name) # need update for different instance
    restaurant_location_info_file_path = os.path.join(Configs.medium_instance_folder_path, restaurant_info_file_name) # need update for different instance
    instance_folder_path = os.path.join(Configs.medium_instance_folder_path, instance_folder) # need update for different instance
    ...
    initial_datetime = datetime.datetime(now.year, now.month, now.day, 9) # start from 9 AM
    ...
    try:
        ...
        id_to_order, id_to_driver, id_to_restaurant_location, id_to_location = get_initial_data(data_file_path,
                                                                                driver_info_file_path,
...
def simulate(customer_info_file: str, restaurant_info_file: str, instance: str, route_map):
    '''
    运行模拟器
    '''
    simulate_env = __initialize(customer_info_file, restaurant_info_file, route_map, instance)
