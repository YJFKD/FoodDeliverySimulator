...
class SimulateEnvironment(object):
    ...
    def __ini_history(self):
        ...
        for driver_id, driver in self.id_to_driver.items():
            history.add_driver_position_history(driver_id, driver.gps_update_time, driver.current_location_id, self.id_to_driver) # add id_to_driver
    ...
    def run(self):
        ...
        # 单独输出订单的历史状态
        order_update_history = f"{Configs.root_folder_path}/Order_update_history/Instance_medium/Instance_"\
                               f"{len(self.id_to_order)}o_{len(self.id_to_driver)}d_{len(self.id_to_location)}l.json" # path need updated for different instance
        driver_update_history = f"{Configs.root_folder_path}/Driver_update_history/Instance_medium/Instance_"\
                                f"{len(self.id_to_order)}o_{len(self.id_to_driver)}d_{len(self.id_to_location)}l.json" # path need updated for different instance
        write_json_to_file(order_update_history, self.history.get_order_status_history()) 
        write_json_to_file(driver_update_history, self.history.get_driver_position_history())
