...
class History(object):
    ...
    def add_driver_position_history(self, driver_id:str, update_time:int, curr_location_id:str, id_to_driver):
        '''
        ...
        if len(curr_location_id)> 0:
            self.__driver_id_to_node_list[driver_id].append({
                                                             "carrying_orders": len([order.id for order in id_to_driver[driver_id].carrying_orders]),
                                                             "location_id": curr_location_id,
                                                             "update_time": str(datetime.fromtimestamp(update_time).strftime("%m/%d/%Y, %H:%M:%S"))}) 
    ...
    def add_order_status_history(self, order_state:int ,update_time:int, committed_completion_time, order_id:str):
        ...
        self.__order_id_to_status_list[order_id].append({"state": order_state,
                                                         "update_time": 
                                                             str(datetime.fromtimestamp(update_time).strftime("%m/%d/%Y, %H:%M:%S")),
                                                         "committed_completion_time": 
                                                             str(datetime.fromtimestamp(committed_completion_time).strftime("%m/%d/%Y, %H:%M:%S")),
                                                         "order_id": order_id})
            ...
            if driver.destination is not None:
                if driver.destination.arrive_time <= to_time:
                    ...
                    for order in driver.destination.pickup_orders:
                        # add the order creation time
                        self.add_order_status_history(Configs.ORDER_STATUS_TO_CODE.get("GENERATED"),
                                                      order.creation_time,
