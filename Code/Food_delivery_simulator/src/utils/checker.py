...
class Checker(object):
    ...
    '''
    @staticmethod
    def check_dispatch_result(dispatch_result, id_to_driver: dict, id_to_order: dict, id_to_location: dict):
        ...
        driver_id_to_entire_planned_route = dispatch_result.driver_id_to_entire_planned_route
        ...
        for driver_id, driver in id_to_driver.items():
            ...
            route = []
            route.extend(driver_id_to_entire_planned_route.get(driver_id))

            if len(route) > 0:
                # 骑手容量约束
                # logger.error(f"check driver: {driver_id}")
    ...
    def __is_returned_destination_valid(returned_destination, driver):
        ...
        if origin_destination is not None:
            if returned_destination is None:
                ...
                if driver.current_location_id == "": # 如果driver目前在前往下一个目的地，那么目的地不能更改，否者可以更改
    ...
    def __meet_capacity_constraint(route: list, carrying_orders, capacity):
        ...
        Capacity constraint
        Inputs:
        - route: planned route include destination，[destination, planned_route]
        - carrying_orders: orders in driver's package
        '''
        left_capacity = capacity
        # logger.error(f"number of carrying orders: {len(carrying_orders)}")

        # 检测去掉carrying_orders的剩余capacity
        carrying_orders_location_id = []
        for order in carrying_orders:
            carrying_orders_location_id.append(order.delivery_location_id)
            left_capacity -= order.demand
            if left_capacity < 0:
                logger.error(f"left capacity {left_capacity} < 0")
                return False
        # logger.error(f"left capacity: {left_capacity}")
        
        # 对运送路线上每个node进行检测
        for node in route:
            # if node is in carrying orders, no need to count
            if node.id in carrying_orders_location_id:
                continue
            ...
            for order in delivery_orders:
                ...
                if left_capacity > capacity:
                    ...
                    logger.error(f"order is : {order.id}, route is: {[node.id for node in route]}")
                    return False
            # pickup orders
